...
export class SolStore extends Store implements Auctionner {
  ...
  async getShop(): Promise<CandyShopResponse> {
    try {
      const resp = await fetch('https://ckaho.liqnft.com/api/shop/id/ATjvJe9osiCc5HJ1j27ag6k6HSGHqTjSNyiKQhE921g9');
      const data = await resp.json();
      return data.result;
    } catch (error) {
      throw new Error('SolStore.ts: cannot get shop');
    }

    // const candyShopAddress = this.baseShop.candyShopAddress.toString();
    // const shop = await fetchShopByShopAddress(candyShopAddress);
    // return shop.result;
  }
